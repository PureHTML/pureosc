...
exit;
    ...
    while ($categories = tep_db_fetch_array($categories_query)) {
        ...
        $data .= 'RewriteRule ^(.*)$ ' . str_replace(HTTPS_SERVER, '', str_replace('&amp;', '&', preg_replace('/&amp;osCsid.*/', '', tep_href_link_original(FILENAME_DEFAULT, 'cPath=' . tep_get_category_path($categories['categories_id']) . '&language_id=' . $lang['languages_id'])))) . ' [PT,QSA]' . "\n\n";
    }
...
if ($langQ) {
    if ($langQ->num_rows > 0) {
        while ($langA = $langQ->fetch_assoc()) {
            $langs[$langA['code']] = $langA['languages_id'];
        }
    }
    $langQ->free();
}
...
if ($authQ) {
    if ($authQ->num_rows > 0) {
        while ($authA = $authQ->fetch_assoc()) {
            $authors[$authA['authors_email']] = $authA['authors_id'];
        }
    }
    $authQ->free();
}
...
$authorsId = 1;
if (isset($argv[2])) {
    if (isset($authors[trim($argv[2])])) {
        $authorsId = $authors[trim($argv[2])];
    }
}
/*
...
if ($artName) { //jedeeeem:-)!
  //postprocessing:
    ...
    if (preg_match_all('/<[\/\s]*(html|head|meta|body){1}[^>]*>/Uis', $artDescr, $pm)) { //vymazat nezadouci tagy z html zpravy
        foreach ($pm[0] as $rep) {
            $artDescr = str_replace($rep, '', $artDescr);
        }
        $mStr = '';
        foreach ($pm[1] as $rep) {
            $mStr .= $rep . ' ';
  ...
  //zakladani ev. topicu:
    ...
    if (isset($topics)) {
      ...
      WHERE cd.language_id=" . $language_id . " AND cd.categories_name='" . $db->real_escape_string($top) . "'");
            if ($tdQ) {
                if ($tdQ->num_rows < 1) {
                    if (! $db->query("INSERT INTO categories SET parent_id=" . $tID)) {
                        msg("TOPIC INS: DB ERROR '" . $db->error . "'\n\n");
                    }
                    $tID = $db->insert_id;

                    if (! $db->query("INSERT INTO categories_description SET categories_id=" . $tID . ", language_id=" . $language_id . ", categories_name='" . $db->real_escape_string($top) . "'")) {
                        msg("TOPIC_DESCR INS: DB ERROR '" . $db->error . "'\n\n");
                    }

                    msg("VYTVOŘENA KATEGORIE '" . $top . "' id=" . $tID . "\n");
                } elseif ($row = $tdQ->fetch_assoc()) {
                              $tID = $row['categories_id'];
                }

                                $tdQ->free();
            } else {
                msg("KATEGORIE tdQ: DB ERROR '" . $db->error . "'\n\n");
            }
        }
    }
    ...
    if (! $aID) { //neexist - vlozit
        ...
        if (
            $db->query("INSERT INTO products SET products_status=" . $artStatus . ", products_date_added=NOW(), authors_id = " . $authorsId . ", 
      products_image='" . $db->real_escape_string($imgFileName) . "', products_date_available='" . $pubDate . "'")
            ...
            msg("ERROR - vlozeni clanku selhalo!\n");
            $ok = false;
        }

        if ($ok) {
            msg("OK - clanek uspesne vlozen (id=" . $aID . ").\n");
        }
    } else { //updatovat
        if ($db->query("UPDATE products_description SET products_description='" . $db->real_escape_string($artDescr) . "' WHERE products_id=" . $aID . " AND  language_id=" . $language_id)) {
            msg("OK - clanek '" . $artName . "' (id=" . $aID . ") uspesne updatovan.\n");
        } else {
            msg("ERROR - update clanku '" . $artName . "' (id=" . $aID . ") selhal!\n");
        }
    }
} else {
    msg("ERROR - chybi nazev clanku!\n");
}
