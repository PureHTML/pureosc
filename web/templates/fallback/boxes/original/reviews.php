...
<?php
  ...
  $corner_left = 'square';
  $corner_right = 'square';
  $boxLink = '<a class="BoxesInfoBoxHeadingCenterBoxRight" title="Box_reviews_grafic_Details" href="' . tep_href_link(FILENAME_REVIEWS) . '">&nbsp;&raquo;&nbsp;</a>';
...
// display random review box
    ...
    $boxContent = '<a accesskey="R" href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' . $random_product['products_id'] . SEPARATOR_LINK . 'reviews_id=' . $random_product['reviews_id']) . '">' . '&nbsp;[R]&nbsp;' . tep_image(DIR_WS_IMAGES . $random_product['products_image'], $random_product['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br /><a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' . $random_product['products_id'] . SEPARATOR_LINK . 'reviews_id=' . $random_product['reviews_id']) . '">' . $rand_review_text . ' ..</a><br />' . tep_image(bts_select(images, 'stars_' . $random_product['reviews_rating'] . '.png'), sprintf(BOX_REVIEWS_TEXT_OF_5_STARS, $random_product['reviews_rating']));
  } elseif (isset($_GET['products_id'])) {
// display 'write a review' box
    $boxContent = '<a accesskey="R" href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, 'products_id=' . $_GET['products_id']) . '">' . '&nbsp;[R]&nbsp;' . tep_image(bts_select(images, 'box_write_review.png'), IMAGE_BUTTON_WRITE_REVIEW) . '</a><br /><a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, 'products_id=' . $_GET['products_id']) . '">' . BOX_REVIEWS_WRITE_REVIEW .'</a>';
  } else {
