         ...
         if ( TRAFFIC_LIGHT == 'false') {
            $lc_text = '<br />&nbsp;' . $listing['products_quantity'] . '&nbsp;<br /><br />';
                           ...
                           $prod_quantity = $listing['products_quantity'];
                            ...
                            case $minus: 
                            $img = tep_image(bts_select(images, 'icon_status_red.png'), TEXT_NOT_AVAIBLE) . '<br />' . TEXT_NOT_AVAIBLE ; 
                            break;  
                            case $red: 
                            $img = tep_image(bts_select(images, 'icon_status_red.png'), TEXT_NOT_AVAIBLE) . '<br />' . TEXT_NOT_AVAIBLE ; 
                            break; 
                            case $yellow : 
                            $img = tep_image(bts_select(images, 'icon_status_yellow.png'), TEXT_FEW_QTY) . '<br />' . TEXT_FEW_QTY ; 
                            break;
                            case $green : 
                            $img = tep_image(bts_select(images, 'icon_status_green.png'), TEXT_BIG_QTY) . '<br />' . TEXT_BIG_QTY ; 
                            break; 
                            } 
            $lc_text =  $img ;
            } 
// image for QTY END
          ...
          case 'PRODUCT_LIST_IMAGE':
            if (isset($_GET['manufacturers_id'])) {
              $lc_text = '<br /><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'manufacturers_id=' . $_GET['manufacturers_id'] . SEPARATOR_LINK . 'products_id=' . $listing['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $listing['products_image'], $listing['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br /><br /><a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now' . SEPARATOR_LINK . 'products_id=' . $listing['products_id']) . '">' . tep_image_button('button_buy_now.png', IMAGE_BUTTON_BUY_NOW) . '</a><br />';
            } else {
              $lc_text = '<br />&nbsp;<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . SEPARATOR_LINK : '') . 'products_id=' . $listing['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $listing['products_image'], $listing['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br /><br /><a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now' . SEPARATOR_LINK . 'products_id=' . $listing['products_id']) . '">' . tep_image_button('button_buy_now.png', IMAGE_BUTTON_BUY_NOW) . '</a><br />';
            }
            break;
          case 'PRODUCT_LIST_BUY_NOW':
            $lc_text = '<br /><br />' . tep_draw_separator();
            break;
        }        
        
if ($lc_text == ('<br /><br />' . tep_draw_separator() ))  { 
