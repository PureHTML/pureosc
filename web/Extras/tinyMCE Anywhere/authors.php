...
// START tinyMCE
$languages = tep_get_languages(); // Moved here so we can build the list of textareas to replace by tinyMCE
if ($action == 'edit') {
		// Build list of textareas to convert
	    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
		  $mce_str.="authors_description[".$languages[$i]['id']."],";
...
<?php
//  $languages = tep_get_languages(); moved up for TinyMce
  for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
?>
          <tr>
