  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'deleteconfirm':
        ...
        break;
      case 'insert':
        if (isset($_POST['folder_name']) && tep_not_null(basename($_POST['folder_name'])) && mkdir($current_path . '/' . basename($_POST['folder_name']), 0777)) {
          tep_redirect(tep_href_link(FILENAME_FILE_MANAGER, 'info=' . urlencode($_POST['folder_name'])));
        }
        break;
      case 'save':
        if (isset($_POST['filename']) && tep_not_null(basename($_POST['filename']))) {
          if (is_writeable($current_path) && ($fp = fopen($current_path . '/' . basename($_POST['filename']), 'w+'))) {
            fputs($fp, stripslashes($_POST['file_contents']));
            fclose($fp);
            tep_redirect(tep_href_link(FILENAME_FILE_MANAGER, 'info=' . urlencode(basename($_POST['filename']))));
          }
        } else {
          $action = 'new_file';
...
<!-- body_text //-->
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td width="100%"><?php echo tep_draw_form('goto', FILENAME_FILE_MANAGER, '', 'get'); ?><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="pageHeading" align="right"><?php echo tep_draw_pull_down_menu('goto', $goto_array, $current_path, 'onchange="this.form.submit();"'); ?><?php echo tep_hide_session_id(); ?></td>
          </tr>
        </table></form></td>
      </tr>
<?php
...
?>
      ...
      <tr>
        <td><?php echo tep_draw_form('new_file', FILENAME_FILE_MANAGER, 'action=save'); ?><table border="0" cellspacing="0" cellpadding="2">
          <tr>
          ...
          <tr>
            <td align="right" class="main" colspan="2"><?php if ($file_writeable == true) echo tep_image_submit('button_save.png', IMAGE_SAVE) . '&nbsp;'; echo '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>'; ?></td>
          </tr>
        </table></form></td>
      </tr>
<?php
...
<?php
  for ($i=0, $n=sizeof($contents); $i<$n; $i++) {
    ...
    if ($contents[$i]['is_dir']) {
      if ($contents[$i]['name'] == '..') {
        $icon = tep_image(DIR_WS_ICONS . 'previous_level.png', ICON_PREVIOUS_LEVEL);
      } else {
        $icon = (isset($fInfo) && is_object($fInfo) && ($contents[$i]['name'] == $fInfo->name) ? tep_image(DIR_WS_ICONS . 'current_folder.png', ICON_CURRENT_FOLDER) : tep_image(DIR_WS_ICONS . 'folder.png', ICON_FOLDER));
      }
      $link = tep_href_link(FILENAME_FILE_MANAGER, 'goto=' . $goto_link);
    } else {
      $icon = tep_image(DIR_WS_ICONS . 'file_download.png', ICON_FILE_DOWNLOAD);
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if ($contents[$i]['name'] != '..') echo '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, 'info=' . urlencode($contents[$i]['name']) . '&amp;action=delete') . '">' . tep_image(DIR_WS_ICONS . 'delete.png', ICON_DELETE) . '</a>&nbsp;'; if (isset($fInfo) && is_object($fInfo) && ($fInfo->name == $contents[$i]['name'])) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.png'); } else { echo '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, 'info=' . urlencode($contents[$i]['name'])) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
              <tr>
                <td colspan="7"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr valign="top">
                    <td class="smallText"><?php echo '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, 'action=reset') . '">' . tep_image_button('button_reset.png', IMAGE_RESET) . '</a>'; ?></td>
                    <td class="smallText" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&amp;' : '') . 'action=upload') . '">' . tep_image_button('button_upload.png', IMAGE_UPLOAD) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&amp;' : '') . 'action=new_file') . '">' . tep_image_button('button_new_file.png', IMAGE_NEW_FILE) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) . '&amp;' : '') . 'action=new_folder') . '">' . tep_image_button('button_new_folder.png', IMAGE_NEW_FOLDER) . '</a>'; ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
    ...
    switch ($action) {
      case 'delete':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_FILE_MANAGER, (tep_not_null($fInfo->name) ? 'info=' . urlencode($fInfo->name) : '')) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
        break;
      case 'new_folder':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' . (($directory_writeable == true) ? tep_image_submit('button_save.png', IMAGE_SAVE) : '') . ' <a href="' . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
        break;
      case 'upload':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' . (($directory_writeable == true) ? tep_image_submit('button_upload.png', IMAGE_UPLOAD) : '') . ' <a href="' . tep_href_link(FILENAME_FILE_MANAGER, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
        break;
      default:
        if (isset($fInfo) && is_object($fInfo)) {
          $heading[] = array('text' => '<b>' . $fInfo->name . '</b>');

          if (!$fInfo->is_dir) $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_FILE_MANAGER, 'info=' . urlencode($fInfo->name) . '&amp;action=edit') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a>');
