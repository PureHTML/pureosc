  ...
  function tep_href_link($page = '', $parameters = '', $connection = 'NONSSL') {
    ...
    if ($connection == 'NONSSL') {
      ...
      if (ENABLE_SSL == 'true') {
        ...
        $link = META_HTTP_SERVER . DIR_WS_ADMIN;
      }
    } else {
    ...
    while ( (substr($link, -1) == '&') || (substr($link, -1, 5) == '&amp;') || (substr($link, -1) == '?') ) $link = substr($link, 0, -1);
...
*/
    $image_submit = '<input class="submit" type="submit" value="' . tep_output_string($alt) . '"';

// end modify for button text
...
if (WEB_EDITOR_LIST == 'fckeditor'){
  require("fckeditor/fckeditor.php"); //orig: "fckeditor/fckeditor.php"
  ////
  // Output a form textarea field w/ fckeditor
    function tep_draw_fckeditor($name, $text) {
    ...
    function tep_draw_fckeditorCd($name, $text) {
	    $oFCKeditor = new FCKeditor($name);
