...
// multi image
  function tep_info_image_extra($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_IMAGES . 'images_extra/' . $image)) ) {
      $image = tep_image(DIR_WS_CATALOG_IMAGES . 'images_extra/' . $image, $alt, $width, $height);
    }else {
      $image = TEXT_IMAGE_NONEXISTENT;
    }
    return $image;
...
/* todo
  function tep_get_categories_description($categories_id, $language_id) {
    $categoryd_query = tep_db_query("select categories_description from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$language_id . "'");
    $categoriesd = tep_db_fetch_array($categoryd_query);

    return $categoriesd['categories_description'];
  }
*/
...
// Retreive server information
  ...
  function tep_remove_product($product_id) {
    ...
    for($nb=1; $nb <= NB_IMAGE_EXTRA ; $nb++) {
      $product_img_to_delete = tep_db_query("select products_image".$nb . " from " . TABLE_PRODUCTS_IMAGES . " where products_id = '" . (int)$product_id . "'");
...
function tep_reset_cache_data_seo_urls($action){        
        switch ($action){
                case 'reset':
                        tep_db_query("DELETE FROM " . TABLE_CACHE_SEO . " WHERE cache_name LIKE '%seo_urls%'");
