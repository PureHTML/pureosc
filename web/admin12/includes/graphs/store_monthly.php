<?php
  ...
  $store_stats_query = tep_db_query("select month(store_history_date) as store_month, sum(store_shown) as value, sum(store_clicked) as dvalue from " . TABLE_STORE_HISTORY . " where year(store_history_date) = '" . $year . "' group by store_month");
  ...
  touch(DIR_FS_ADMIN . 'images/graphs/store_monthly_' . $graph_type . '.' . $store_extension); // Create blank file
  chmod(DIR_FS_ADMIN . 'images/graphs/store_monthly_' . $graph_type . '.' . $store_extension,0777);
  $graph = new PHPlot(600, 350, 'images/graphs/store_monthly_' . $graph_type . '.' . $store_extension);
