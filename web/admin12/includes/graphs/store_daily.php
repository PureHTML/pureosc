<?php
  ...
  $store_stats_query = tep_db_query("select dayofmonth(store_history_date) as store_day, store_shown as value, store_clicked as dvalue from " . TABLE_STORE_HISTORY . " where  month(store_history_date) = '" . $month . "' and year(store_history_date) = '" . $year . "'");
  ...
  @touch(DIR_FS_ADMIN . 'images/graphs/store_daily_' . $graph_type . '.' . $store_extension); // Create blank file
  @chmod(DIR_FS_ADMIN . 'images/graphs/store_daily_' . $graph_type . '.' . $store_extension,0777);
  $graph = new PHPlot(600, 350, 'images/graphs/store_daily_' . $graph_type . '.' . $store_extension);
