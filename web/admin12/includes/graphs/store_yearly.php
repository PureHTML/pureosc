<?php
  ...
  $store_stats_query = tep_db_query("select year(store_history_date) as year, sum(store_shown) as value, sum(store_clicked) as dvalue from " . TABLE_STORE_HISTORY . " group by year");
  ...
  touch(DIR_FS_ADMIN . 'images/graphs/store_yearly_' . $graph_type . '.' . $store_extension); // Create blank file
  chmod(DIR_FS_ADMIN . 'images/graphs/store_yearly_' . $graph_type . '.' . $store_extension,0777);
  $graph = new PHPlot(600, 350, 'images/graphs/store_yearly_' . $graph_type . '.' . $store_extension);
