...
<?php
  ...
  switch ($_GET['action']) {
    case 'release':
      ...
      $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link('gv_queue.php','action=confirmrelease&amp;gid='.$gInfo->unique_id,'NONSSL').'">'.tep_image_button('button_confirm_red.png', IMAGE_CONFIRM) . '</a> <a href="' . tep_href_link('gv_queue.php','action=cancel&amp;gid=' . $gInfo->unique_id,'NONSSL') . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    default:
      $heading[] = array('text' => '[' . $gInfo->unique_id . '] ' . tep_datetime_short($gInfo->date_created) . ' ' . $currencies->format($gInfo->amount));

      $contents[] = array('align' => 'center','text' => '<a href="' . tep_href_link('gv_queue.php','action=release&amp;gid=' . $gInfo->unique_id,'NONSSL'). '">' . tep_image_button('button_release.png', IMAGE_RELEASE) . '</a>');
      break;
   }
