        ...
        if (isset($customer_id)) $cust_id=$customer_id;
  		 ...
  		 $current_page=addslashes(tep_db_input(urldecode($_SERVER['PHP_SELF'])));
    		 ...
    		 $query_string=$refer_data[1];
    		 	//MB: Add Agent tracking (note: only adding agent for new visitor)
    		 	if (isset($_GET['rep'])) {
    		 		$rep = $_GET['rep'];
    		 	}
         ...
         geoip_close($gi);		  	
    		 ...
    		 //MB: Add agent tracking, update insert statement to include agent data
         $query = "INSERT INTO `supertracker` (`ip_address`, `browser_string`, `country_code`, `country_name`, `referrer`,`referrer_query_string`,`landing_page`,`exit_page`,`time_arrived`,`last_click`, `agent`) VALUES ('" . $ip . "','" . $browser_string . "','" . $country_code . "', '" . $country_name . "', '" . $referrer . "', '" . $query_string . "','" . $landing_page . "','" . $current_page . "','" . $time_arrived . "','" . $time_arrived . "','" . $rep . "')";		 
    		 tep_db_query($query);
