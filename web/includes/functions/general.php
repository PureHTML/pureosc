    ...
    if (is_array($_GET) && (sizeof($_GET) > 0)) {
      ...
      while (list($key, $value) = each($_GET)) {
        if ( (strlen($value) > 0) && ($key != tep_session_name()) && ($key != 'error') && (!in_array($key, $exclude_array)) && ($key != 'x') && ($key != 'y') ) {
          $get_url .= $key . '=' . rawurlencode(stripslashes($value)) . SEPARATOR_LINK;
        }
      }
    }
...
//      $address = $company . $cr . $address;
      $address = $company . $cr . $address. $cr . "P.I. ". $piva;
      //PIVACF end
...
//    $address_query = tep_db_query("select entry_firstname as firstname, entry_lastname as lastname, entry_company as company, entry_street_address as street_address, entry_suburb as suburb, entry_city as city, entry_postcode as postcode, entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$customers_id . "' and address_book_id = '" . (int)$address_id . "'");
	$address_query = tep_db_query("select entry_firstname as firstname, entry_lastname as lastname, entry_company as company, entry_street_address as street_address, entry_suburb as suburb, entry_city as city, entry_postcode as postcode, entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id, entry_piva as piva,  entry_cf as cf from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$customers_id . "' and address_book_id = '" . (int)$address_id . "'");
    //PIVACF end
  ...
  function tep_row_number_format_9($number) {
    if ( ($number < 10) && (substr($number, 0, 1) != '0') ) $number =  $number;
...
//                    e.g. info@mytepshop.com
  ...
  function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address) {
    ...
    if (EMAIL_USE_HTML == 'true') {
      ...
      $message->add_text(html_entity_decode($text));
    }
...
// Get the number of times a word/character is present in a string
  ...
  function tep_array_to_string($array, $exclude = '', $equals = '=', $separator = SEPARATOR_LINK) {
...
// nl2br() prior PHP 4.2.0 did not convert linefeeds on all OSs (it only converted \n)
  ...
  // stat v3
  function count_store_viewed($ip_address){
     $viewed_info_query = tep_db_query("select * from " . TABLE_STORE_VIEWED . " where ip_address='" . $ip_address . "' and date_format(store_viewed_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
     $viewed_info = tep_db_fetch_array($viewed_info_query);

     $today = getdate();

     if(tep_db_num_rows($viewed_info_query)==0){
        $sql_data_array = array('ip_address' => $ip_address,
                                'store_viewed_date' =>'now()');
        tep_db_perform(TABLE_STORE_VIEWED, $sql_data_array);

        $store_check_query = tep_db_query("select count(*) as count from " . TABLE_STORE_HISTORY . " where  date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
        $store_check = tep_db_fetch_array($store_check_query);
        if ($store_check['count'] > 0) {
          tep_db_query("update " . TABLE_STORE_HISTORY . " set store_clicked = store_clicked + 1 where  date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
        } else {
          tep_db_query("insert into " . TABLE_STORE_HISTORY . " (store_clicked, store_shown, store_history_date) values (1, 1, now())");
        }
        ...
        $store_check = tep_db_fetch_array($store_check_query);
        if ($store_check['count'] == 0) {
           tep_db_query("insert into " . TABLE_STORE_HISTORY . " (store_shown, store_history_date) values (1, now())");
        }else{
          tep_db_query("update " . TABLE_STORE_HISTORY . " set store_shown = store_shown + 1 where  date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
        }
     }

     tep_db_query("delete from " . TABLE_STORE_VIEWED . " where date_format(store_viewed_date, '%Y%m%d') != date_format(now(), '%Y%m%d')");
  }

  function count_store_orders(){
    global $order,$order_totals;
        for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
            $amount_products += $order->products[$i]['qty'];
        }
        $orders_price = $order_totals[sizeof($order_totals)-1]['value'];

        tep_db_query("update " . TABLE_STORE_HISTORY . " set amount_orders = amount_orders + 1, amount_products = amount_products+" . $amount_products . ", orders_price=orders_price+" . $orders_price . " where  date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
  }

  function count_store_products_viewed($products_id){
        $store_check_query = tep_db_query("select count(*) as count from " . TABLE_STORE_PRODUCTS_HISTORY . " where products_id=" . $products_id . " and date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
        $store_check = tep_db_fetch_array($store_check_query);
        if ($store_check['count'] == 0) {
           tep_db_query("insert into " . TABLE_STORE_PRODUCTS_HISTORY . " (store_products_shown, store_history_date, products_id) values (1, now()," . $products_id. ")");
        }else{
          tep_db_query("update " . TABLE_STORE_PRODUCTS_HISTORY . " set store_products_shown = store_products_shown + 1 where  products_id=" . $products_id . " and date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
        }
  }

  function count_store_products_purchased($products_id,$qty=1){
        $store_check_query = tep_db_query("select count(*) as count from " . TABLE_STORE_PRODUCTS_HISTORY . " where products_id=" . $products_id . " and date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
        $store_check = tep_db_fetch_array($store_check_query);
        if ($store_check['count'] == 0) {
           tep_db_query("insert into " . TABLE_STORE_PRODUCTS_HISTORY . " (store_products_purchased, store_history_date, products_id) values (1, now()," . $products_id. ")");
        }else{
          tep_db_query("update " . TABLE_STORE_PRODUCTS_HISTORY . " set store_products_purchased = store_products_purchased + " . $qty . " where  products_id=" . $products_id . " and date_format(store_history_date, '%Y%m%d') = date_format(now(), '%Y%m%d')");
        }
  }
  // end
...
// BOF ID FILTER
  function tep_real_escape_array(&$array, $overflow=0) {
    if($overflow > 10 )
      exit();
    foreach ($array as $key => $value) {
      if (is_array($value)) {
        $overflow++;
        $array[$key] = tep_real_escape_array($value, $overflow);
        $overflow--;
      }
      else {
        // Stripslashes
...
function information_page_not_found() {
 ...
 function print_pre ($argument,$comment=''){
	...
	if($html)
		echo '<pre class="debug" style="overflow: scroll; border: solid 1px green;  color: white; background-color: black;" title="'.$comment.'">';
	else	
		echo "\n########### $comment ###########\n";
...
function tep_flatten_product_description_hp($description, $link)
{
  $description = ereg_replace('<[^>]*>', '', $description);
  if (strlen($description) > PRODUCT_LIST_DESCRIPTION_HP_MAX_LENGTH){
    $sub_description = wordwrap($description, PRODUCT_LIST_DESCRIPTION_HP_MAX_LENGTH,'$@$');
        $sub_des = explode('$@$',$sub_description);
        $description = $sub_des[0];
        $description .= "...".$link;
      }
...
//shop2.0brain: new 
function tep_flatten_product_description_long($description, $link)
{
  $description = ereg_replace('<[^>]*>', '', $description);
  if (strlen($description) > PRODUCT_LIST_DESCRIPTION_LONG_MAX_LENGTH){
    $sub_description = wordwrap($description, PRODUCT_LIST_DESCRIPTION_LONG_MAX_LENGTH,'$@$');
        $sub_des = explode('$@$',$sub_description);
        $description = $sub_des[0];
        $description .= "...".$link;
      }
...
function tep_flatten_product_description_xml($description, $desclenght)
{
  $description = ereg_replace('<[^>]*>', '', $description);
  if (strlen($description) > $desclenght){
    $sub_description = wordwrap($description, $desclenght,'$@$');
        $sub_des = explode('$@$',$sub_description);
        $description = $sub_des[0];
      }
  return $description;
...
function manufacturer_author($manufacturers_name_orig)
{
$tittleh1a = ereg_replace('\,.*','',$manufacturers_name_orig);
$tittleh1b = ereg_replace('^.*\,','',$manufacturers_name_orig);
//$manufacturers_name_orig = $tittleh1b .' ' .  $tittleh1a;
}
 return $manufacturers_name_orig = $tittleh1b .'haf ' .  $tittleh1a;
?>