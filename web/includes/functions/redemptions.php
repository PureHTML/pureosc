...
// calculate the products shopping points tax value if any  
    ...
    if ((DISPLAY_PRICE_WITH_TAX == 'true') && (USE_POINTS_FOR_TAX == 'true')) {
      $products_price_points_query = $currencies->calculate_price($products_price, $products_tax, $quantity);
      } else {
...
// awards restriction if enabled.
 ...
 function points_selection() {
   ...
   if (($customer_shopping_points = tep_get_shopping_points()) && $customer_shopping_points > 1){
         ...
         if ((POINTS_MIN_AMOUNT == '') || ($cart->show_total() >= POINTS_MIN_AMOUNT) ){
           if (tep_session_is_registered('customer_shopping_points_spending')) tep_session_unregister('customer_shopping_points_spending');
             ...
             if ($order->info['total'] > tep_calc_shopping_pvalue($max_points)) {
	           $note = '<br />' . TEXT_REDEEM_SYSTEM_NOTE;
             }
             $customer_shopping_points_spending = $max_points;
?>
<h2 class="b"><?php echo TABLE_HEADING_REDEEM_SYSTEM; ?></h2>
<div class="InfoBoxContenent2MA">
    <?php printf(TEXT_REDEEM_SYSTEM_START, $currencies->format(tep_calc_shopping_pvalue($customer_shopping_points)), $currencies->format($order->info['total']). $note); ?>
<br /><br />
    <?php echo '<label for="point_spend">';
    ...
    <?php echo tep_draw_checkbox_field('customer_shopping_points_spending', $customer_shopping_points_spending, '', 'id="point_spend" onclick="submitFunction()"'); 
          echo '</label>';
    ?>
</div>
<br />
...
?>
<div class="InfoBoxContenent2MA">
    <span class="b"><?php echo TABLE_HEADING_REFERRAL; ?></span><br />
    <?php echo tep_draw_input_field_label(TEXT_REFERRAL_REFERRED, false, 'customer_referred', '', $customer_referred); ?> </div><br />
<?php
   } 
 }
