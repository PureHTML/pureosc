  ...
  function tep_display_banner($action, $identifier) {
    if ($action == 'dynamic') {
      ...
      if ($banners['count'] > 0) {
        ...
        return '<span class="b">TEP ERROR! (tep_display_banner(' . $action . ', ' . $identifier . ') -> No banners with group \'' . $identifier . '\' found!</span>';
      }
      ...
      if (is_array($identifier)) {
        ...
        if (tep_db_num_rows($banner_query)) {
          ...
          return '<span class="b">TEP ERROR! (tep_display_banner(' . $action . ', ' . $identifier . ') -> Banner with ID \'' . $identifier . '\' not found, or status inactive</span>';
        }
      }
    } else {
      return '<span class="b">TEP ERROR! (tep_display_banner(' . $action . ', ' . $identifier . ') -> Unknown $action parameter value - it must be either \'dynamic\' or \'static\'</span>';
    }

    if (tep_not_null($banner['banners_html_text'])) {
      $banner_string = $banner['banners_html_text'];
    } else {
      $banner_string = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner' . SEPARATOR_LINK . 'goto=' . $banner['banners_id']) . '" >' . tep_image(DIR_WS_IMAGES . $banner['banners_image'], $banner['banners_title']) . '</a>';
    }
