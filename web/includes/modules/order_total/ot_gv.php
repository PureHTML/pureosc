       ...
       if (tep_session_is_registered('cot_gv')) {
        ...
        if ($od_amount > 0) {
          $this->output[] = array('title' => $this->title . ':',
                           'text' => '<span class="b">' . $currencies->format($od_amount) . '</span><br /><h2 class="ColorRed"> ---- = ' . $currencies->format($order_total - $od_amount ).  ' = ---- </h2>',
                           'value' => $od_amount);
        }
      }
    }
     ...
     global $currencies;
       ...
       if ($my_od_amount > 0) {
         $this->my_output[] = array('title' => $this->title . ':',
                          'text' => '<span class="b">' . $currencies->format($my_od_amount) . '</span>',
                          'value' => $my_od_amount);
       }
   }
...
// if there has been something before us then
 ...
 // #################### Begin Added CGV JONYO ######################
	...
	function use_credit_amount_sub() {
      ...
      $cot_gv = false;
      if ($this->selection_test()) {
        $output_string .= '<span class="b">' . $this->checkbox . '</span>' . "\n";
      }
      return $output_string;
    }
 // #################### End Added CGV JONYO ######################
    ...
    function credit_selection() {
      ...
      if (tep_db_num_rows($gv_query)) {
        ...
        $selection_string .= tep_image_submit('button_redeem.png', IMAGE_REDEEM_VOUCHER, 'onclick="submitFunction()"');
      }
