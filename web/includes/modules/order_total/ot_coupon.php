...
global $PHP_SELF, $order, $currencies;
        ...
        if ($od_amount > 0) {
                ...
                $this->output[] = array('title' => $this->title . ' : ' . $this->coupon_code .' =',
                                        'text'  => '<span class="b">-' . $currencies->format($od_amount) . '</span>', 
...
global $customer_id, $currencies, $language; 
        ...
        $selection_string .= '<br /><div class="InfoBoxContenent2MA"><br />';
        $selection_string .= tep_draw_input_field_label(TEXT_ENTER_GV_CODE, true, 'gv_redeem_code', TEXT_ENTER_GV_CODE) ;
        $selection_string .= tep_image_submit('button_redeem.png', IMAGE_REDEEM_VOUCHER, 'onclick="return submitFunction()"');
        $selection_string .= '<br /><br /></div><br /></form>';
...
// END <<< CCVG 5.15 - Custom Modification - fix Coupon code redemption error
                ...
                if ($coupon_result['coupon_minimum_order']>0) $coupon_amount_out .= MODULE_ORDER_GREATER . $currencies->format($coupon_result['coupon_minimum_order']);
