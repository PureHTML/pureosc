...
if (!empty($action)) {
    switch ($action) {
        ...
        case 'update':
            ...
            $newsletter_module = basename(tep_db_prepare_input($_POST['module']));
...
<?php
  if ($action === 'new') {
      ...
      <tr><?php echo tep_draw_form('newsletter', 'newsletters.php', (isset($_GET['page']) ? 'page='.$_GET['page'].'&' : '').'action='.$form_action);
 ...
 ?>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td class="smallText" align="right"><?php echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('newsletters.php', (isset($_GET['page']) ? 'page='.$_GET['page'].'&' : '').(isset($_GET['nID']) ? 'nID='.$_GET['nID'] : ''))); ?></td>
          </tr>
        </table></td>
      </form></tr>
<?php
...
<?php
      ...
      while ($newsletters = tep_db_fetch_array($newsletters_query)) {
          ...
          ?>
                ...
                <td class="dataTableContent" align="center"><?php if ($newsletters['status'] === '1') {
...
<?php
      ...
      switch ($action) {
          case 'delete':
              ...
              $contents = ['form' => tep_draw_form('newsletters', 'newsletters.php', 'page='.$_GET['page'].'&nID='.$nInfo->newsletters_id.'&action=deleteconfirm')];
              $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
              $contents[] = ['text' => '<br /><strong>'.$nInfo->title.'</strong>'];
              $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('newsletters.php', 'page='.$_GET['page'].'&nID='.$_GET['nID']))];
