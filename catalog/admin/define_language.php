...
for ($i = 0, $n = \count($languages); $i < $n; ++$i) {
    ...
    $languages_array[] = ['id' => $languages[$i]['directory'],
      ...
      if (file_exists($file)) {
          ...
          <tr><?php echo tep_draw_form('language', 'define_language.php', 'lngdir='.$_GET['lngdir'].'&filename='.$_GET['filename'].'&action=save'); ?>
            <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
              ...
              <tr>
                <td class="smallText" align="right"><?php if ($file_writeable === true) {
                    echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('define_language.php', 'lngdir='.$_GET['lngdir']));
                } else {
