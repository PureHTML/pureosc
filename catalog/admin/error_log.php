...
$log_date = null;
$log_date_array = [['id' => '', 'text' => TEXT_ALL_LOG]];

foreach ($log_files as $file) {
    preg_match('/errors-(\d+).txt/', $file, $matches);

    $log_date_array[] = ['id' => $matches[1], 'text' => date(DATE_FORMAT, strtotime($matches[1]))];
}
...
<?php
                  foreach (array_reverse($log_files) as $file) {
                      ...
                      <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">
                        <td class="dataTableContent"><?php echo str_replace([DIR_FS_CATALOG, ' '.CFG_TIME_ZONE], '', $error); ?></td>
                      </tr>
<?php
                      }
                  }
