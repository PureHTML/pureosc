...
function popupImageWindow(url) {
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
        ...
        if ($action === 'new') {
          ...
          <tr><?php echo tep_draw_form('new_banner', FILENAME_BANNER_MANAGER, (isset($HTTP_GET_VARS['page']) ? 'page='.$HTTP_GET_VARS['page'].'&' : '').'action='.$form_action, 'post', 'enctype="multipart/form-data"');
 ...
 ?>
            <td><table border="0" cellspacing="0" cellpadding="2">
                ...
                <tr>
                  ...
                  <td class="main"><?php echo tep_draw_pull_down_menu('banners_group', $groups_array, $bInfo->banners_group).TEXT_BANNERS_NEW_GROUP.'<br />'.tep_draw_input_field('new_banners_group', '', '', (\count($groups_array) > 0) ? false : true); ?></td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td class="main"><?php echo tep_draw_file_field('banners_image').' '.TEXT_BANNERS_IMAGE_LOCAL.'<br />'.DIR_FS_CATALOG_IMAGES.tep_draw_input_field('banners_image_local', $bInfo->banners_image ?? ''); ?></td>
                </tr>
                <tr>
                  <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                </tr>
    ...
    $('#expires_date').datepicker({
      dateFormat: 'yy-mm-dd'
            ...
            while ($banners = tep_db_fetch_array($banners_query)) {
                ...
                ?></td>
                            <td class="dataTableContent" align="right"><?php echo '<a href="'.tep_href_link(FILENAME_BANNER_STATISTICS, 'page='.$HTTP_GET_VARS['page'].'&bID='.$banners['banners_id']).'">'.tep_image(DIR_WS_ICONS.'statistics.gif', ICON_STATISTICS).'</a>&nbsp;';
 ...
 ?>&nbsp;</td>
    ...
    <?php
            ...
            switch ($action) {
                case 'delete':
                    $heading[] = ['text' => '<strong>'.$bInfo->banners_title.'</strong>'];

                    $contents = ['form' => tep_draw_form('banners', FILENAME_BANNER_MANAGER, 'page='.$HTTP_GET_VARS['page'].'&bID='.$bInfo->banners_id.'&action=deleteconfirm')];
                    $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
                    $contents[] = ['text' => '<br /><strong>'.$bInfo->banners_title.'</strong>'];

                    if ($bInfo->banners_image) {
                        $contents[] = ['text' => '<br />'.tep_draw_checkbox_field('delete_image', 'on', true).' '.TEXT_INFO_DELETE_IMAGE];
                    }

                    $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link(FILENAME_BANNER_MANAGER, 'page='.$HTTP_GET_VARS['page'].'&bID='.$HTTP_GET_VARS['bID']))];
                ...
                default:
                    if (\is_object($bInfo)) {
                        ...
                        if (\function_exists('imagecreate') && $dir_ok && $banner_extension) {
                            ...
                            $contents[] = ['align' => 'center', 'text' => '<br />'.tep_image(DIR_WS_IMAGES.'graphs/banner_infobox-'.$banner_id.'.'.$banner_extension)];
                        } else {
                            include DIR_WS_FUNCTIONS.'html_graphs.php';
                            $contents[] = ['align' => 'center', 'text' => '<br />'.tep_banner_graph_infoBox($bInfo->banners_id, '3')];
                        }

                        $contents[] = ['text' => tep_image(DIR_WS_IMAGES.'graph_hbar_blue.gif', 'Blue', '5', '5').' '.TEXT_BANNERS_BANNER_VIEWS.'<br />'.tep_image(DIR_WS_IMAGES.'graph_hbar_red.gif', 'Red', '5', '5').' '.TEXT_BANNERS_BANNER_CLICKS];

                        if ($bInfo->date_scheduled) {
                            $contents[] = ['text' => '<br />'.sprintf(TEXT_BANNERS_SCHEDULED_AT_DATE, tep_date_short($bInfo->date_scheduled))];
                        }

                        if ($bInfo->expires_date) {
                            $contents[] = ['text' => '<br />'.sprintf(TEXT_BANNERS_EXPIRES_AT_DATE, tep_date_short($bInfo->expires_date))];
                        } elseif ($bInfo->expires_impressions) {
                            $contents[] = ['text' => '<br />'.sprintf(TEXT_BANNERS_EXPIRES_AT_IMPRESSIONS, $bInfo->expires_impressions)];
                        }

                        if ($bInfo->date_status_change) {
                            $contents[] = ['text' => '<br />'.sprintf(TEXT_BANNERS_STATUS_CHANGE, tep_date_short($bInfo->date_status_change))];
                        }
                    }
