...
$zones_split = new split_page_results($_GET['page'], MAX_DISPLAY_SEARCH_RESULTS, $zones_query_raw, $zones_query_numrows);
...
while ($zones = tep_db_fetch_array($zones_query)) {
    ...
    ?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && \is_object($cInfo) && ($zones['zone_id'] === $cInfo->zone_id)) {
                    echo tep_image('images/icon_arrow_right.gif', '');
                } else {
                    echo '<a href="'.tep_href_link('zones.php', 'page='.$_GET['page'].'&cID='.$zones['zone_id']).'">'.tep_image('images/icon_info.gif', IMAGE_ICON_INFO).'</a>';
                }
...
switch ($action) {
    case 'new':
        ...
        $contents[] = ['text' => '<br />'.TEXT_INFO_ZONES_NAME.'<br />'.tep_draw_input_field('zone_name')];
        $contents[] = ['text' => '<br />'.TEXT_INFO_ZONES_CODE.'<br />'.tep_draw_input_field('zone_code')];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_NAME.'<br />'.tep_draw_pull_down_menu('zone_country_id', tep_get_countries())];
        $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('zones.php', 'page='.$_GET['page']))];
    ...
    case 'edit':
        ...
        $contents[] = ['text' => '<br />'.TEXT_INFO_ZONES_NAME.'<br />'.tep_draw_input_field('zone_name', $cInfo->zone_name)];
        $contents[] = ['text' => '<br />'.TEXT_INFO_ZONES_CODE.'<br />'.tep_draw_input_field('zone_code', $cInfo->zone_code)];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_NAME.'<br />'.tep_draw_pull_down_menu('zone_country_id', tep_get_countries(), $cInfo->countries_id)];
        $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('zones.php', 'page='.$_GET['page'].'&cID='.$cInfo->zone_id))];
    ...
    case 'delete':
        ...
        $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('zones.php', 'page='.$_GET['page'].'&cID='.$cInfo->zone_id))];
    ...
    default:
        if (isset($cInfo) && \is_object($cInfo)) {
            ...
            $contents[] = ['align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('zones.php', 'page='.$_GET['page'].'&cID='.$cInfo->zone_id.'&action=edit')).tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('zones.php', 'page='.$_GET['page'].'&cID='.$cInfo->zone_id.'&action=delete'))];
