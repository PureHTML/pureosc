...
$modules_installed = (!empty(MODULE_CONTENT_INSTALLED) ? explode(';', MODULE_CONTENT_INSTALLED) : []);
                    ...
                    if (!is_dir(DIR_FS_CATALOG_MODULES.'content/'.$group.'/'.$file)) {
                        if (substr($file, strrpos($file, '.')) === $file_extension) {
                            ...
                            if (tep_class_exists($class)) {
                                ...
                                if (\in_array($group.'/'.$class, $modules_installed, true)) {
        ...
        case 'remove':
            ...
            foreach ($modules['installed'] as $m) {
                if ($m['code'] === $class) {
                    ...
                    if (\in_array($m['group'].'/'.$m['code'], $modules_installed, true)) {
                        unset($modules_installed[array_search($m['group'].'/'.$m['code'], $modules_installed, true)]);
                    }
...
<?php
          foreach ($modules['new'] as $m) {
              ...
              ?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($mInfo) && \is_object($mInfo) && ($module->code === $mInfo->code)) {
...
switch ($action) {
    case 'edit':
        ...
        $contents[] = ['text' => $keys];
        $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('modules_content.php', 'module='.$mInfo->code))];
    ...
    default:
        if (isset($mInfo)) {
            ...
            if ($action === 'list_new') {
                $contents[] = ['align' => 'center', 'text' => tep_draw_button(IMAGE_MODULE_INSTALL, 'plus', tep_href_link('modules_content.php', 'module='.$mInfo->code.'&action=install'))];
                ...
                $contents[] = ['align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('modules_content.php', 'module='.$mInfo->code.'&action=edit')).tep_draw_button(IMAGE_MODULE_REMOVE, 'minus', tep_href_link('modules_content.php', 'module='.$mInfo->code.'&action=remove'))];
