...
while ($countries = tep_db_fetch_array($countries_query)) {
    ...
    ?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && \is_object($cInfo) && ($countries['countries_id'] === $cInfo->countries_id)) {
                    echo tep_image('images/icon_arrow_right.gif', '');
                } else {
                    echo '<a href="'.tep_href_link('countries.php', 'page='.$_GET['page'].'&cID='.$countries['countries_id']).'">'.tep_image('images/icon_info.gif', IMAGE_ICON_INFO).'</a>';
                }
...
switch ($action) {
    case 'new':
        $heading[] = ['text' => '<strong>'.TEXT_INFO_HEADING_NEW_COUNTRY.'</strong>'];

        $contents = ['form' => tep_draw_form('countries', 'countries.php', 'page='.$_GET['page'].'&action=insert')];
        $contents[] = ['text' => TEXT_INFO_INSERT_INTRO];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_NAME.'<br />'.tep_draw_input_field('countries_name')];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_CODE_2.'<br />'.tep_draw_input_field('countries_iso_code_2')];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_CODE_3.'<br />'.tep_draw_input_field('countries_iso_code_3')];
        $contents[] = ['text' => '<br />'.TEXT_INFO_ADDRESS_FORMAT.'<br />'.tep_draw_pull_down_menu('address_format_id', tep_get_address_formats())];
        $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('countries.php', 'page='.$_GET['page']))];

        break;
    case 'edit':
        $heading[] = ['text' => '<strong>'.TEXT_INFO_HEADING_EDIT_COUNTRY.'</strong>'];

        $contents = ['form' => tep_draw_form('countries', 'countries.php', 'page='.$_GET['page'].'&cID='.$cInfo->countries_id.'&action=save')];
        $contents[] = ['text' => TEXT_INFO_EDIT_INTRO];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_NAME.'<br />'.tep_draw_input_field('countries_name', $cInfo->countries_name)];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_CODE_2.'<br />'.tep_draw_input_field('countries_iso_code_2', $cInfo->countries_iso_code_2)];
        $contents[] = ['text' => '<br />'.TEXT_INFO_COUNTRY_CODE_3.'<br />'.tep_draw_input_field('countries_iso_code_3', $cInfo->countries_iso_code_3)];
        $contents[] = ['text' => '<br />'.TEXT_INFO_ADDRESS_FORMAT.'<br />'.tep_draw_pull_down_menu('address_format_id', tep_get_address_formats(), $cInfo->address_format_id)];
        $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('countries.php', 'page='.$_GET['page'].'&cID='.$cInfo->countries_id))];

        break;
    case 'delete':
        $heading[] = ['text' => '<strong>'.TEXT_INFO_HEADING_DELETE_COUNTRY.'</strong>'];
        ...
        $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('countries.php', 'page='.$_GET['page'].'&cID='.$cInfo->countries_id))];
    ...
    default:
        if (\is_object($cInfo)) {
            ...
            $contents[] = ['align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('countries.php', 'page='.$_GET['page'].'&cID='.$cInfo->countries_id.'&action=edit')).tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('countries.php', 'page='.$_GET['page'].'&cID='.$cInfo->countries_id.'&action=delete'))];
