...
<?php
  if (($action === 'new') || ($action === 'edit')) {
      ...
      ?>
      <tr><form name="new_special" <?php echo 'action="'.tep_href_link('specials.php', tep_get_all_get_params(['action', 'info', 'sID']).'action='.$form_action).'"'; ?> method="post"><?php if ($form_action === 'update') {
      ...
      ?>
        <td><br /><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main"><?php echo (isset($sInfo->products_name)) ? $sInfo->products_name.' <small>('.$currencies->format($sInfo->products_price).')</small>' : tep_draw_products_pull_down('products_id', 'style="font-size:10px"', $specials_array);
      echo tep_draw_hidden_field('products_price', $sInfo->products_price ?? ''); ?></td>
          </tr>
          <tr>
...
<?php
      ...
      switch ($action) {
          case 'delete':
              $heading[] = ['text' => '<strong>'.TEXT_INFO_HEADING_DELETE_SPECIALS.'</strong>'];

              $contents = ['form' => tep_draw_form('specials', 'specials.php', 'page='.$_GET['page'].'&sID='.$sInfo->specials_id.'&action=deleteconfirm')];
              $contents[] = ['text' => TEXT_INFO_DELETE_INTRO];
              $contents[] = ['text' => '<br /><strong>'.$sInfo->products_name.'</strong>'];
              $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('specials.php', 'page='.$_GET['page'].'&sID='.$sInfo->specials_id))];

              break;

          default:
              if (isset($sInfo) && \is_object($sInfo)) {
                  $heading[] = ['text' => '<strong>'.$sInfo->products_name.'</strong>'];

                  $contents[] = ['align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('specials.php', 'page='.$_GET['page'].'&sID='.$sInfo->specials_id.'&action=edit')).tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('specials.php', 'page='.$_GET['page'].'&sID='.$sInfo->specials_id.'&action=delete'))];
                  $contents[] = ['text' => '<br />'.TEXT_INFO_DATE_ADDED.' '.tep_date_short($sInfo->specials_date_added)];
                  $contents[] = ['text' => ''.TEXT_INFO_LAST_MODIFIED.' '.tep_date_short($sInfo->specials_last_modified)];
                  $contents[] = ['align' => 'center', 'text' => '<br />'.tep_info_image('products/thumbs/'.$sInfo->products_image, $sInfo->products_name, SMALL_IMAGE_WIDTH / 2, SMALL_IMAGE_HEIGHT / 2)];
                  $contents[] = ['text' => '<br />'.TEXT_INFO_ORIGINAL_PRICE.' '.$currencies->format($sInfo->products_price)];
                  ...
                  $contents[] = ['text' => '<br />'.TEXT_INFO_EXPIRES_DATE.' <strong>'.tep_date_short($sInfo->expires_date).'</strong>'];
