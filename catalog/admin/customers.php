...
<?php
  if ($action === 'edit' || $action === 'update') {
      $newsletter_array = [['id' => '1', 'text' => ENTRY_NEWSLETTER_YES],
      ...
      <tr><?php echo tep_draw_form('customers', 'customers.php', tep_get_all_get_params(['action']).'action=update', 'post', 'onsubmit="return check_form();"').tep_draw_hidden_field('default_address_id', $cInfo->customers_default_address_id); ?>
...
<?php
      ...
      <tr>
        <td align="right" class="smallText"><?php echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers.php', tep_get_all_get_params(['action']))); ?></td>
      </tr></form>
<?php
  } else {
      ?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr><?php echo tep_draw_form('search', 'customers.php', '', 'get'); ?>
            <td class="pageHeading"><?php echo HEADING_TITLE; ?></td>
...
<?php
      ...
      while ($customers = tep_db_fetch_array($customers_query)) {
          $info_query = tep_db_query("select customers_info_date_account_created as date_account_created, customers_info_date_account_last_modified as date_account_last_modified, customers_info_date_of_last_logon as date_last_logon, customers_info_number_of_logons as number_of_logons from customers_info where customers_info_id = '".(int) $customers['customers_id']."'");
          ...
          if (isset($cInfo) && \is_object($cInfo) && ($customers['customers_id'] === $cInfo->customers_id)) {
              echo '          <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''.tep_href_link('customers.php', tep_get_all_get_params(['cID', 'action']).'cID='.$cInfo->customers_id.'&action=edit').'\'">'."\n";
          } else {
              echo '          <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\''.tep_href_link('customers.php', tep_get_all_get_params(['cID']).'cID='.$customers['customers_id']).'\'">'."\n";
          }
          ...
          ?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && \is_object($cInfo) && ($customers['customers_id'] === $cInfo->customers_id)) {
...
<?php
      ...
      switch ($action) {
          case 'confirm':
              ...
              $contents = ['form' => tep_draw_form('customers', 'customers.php', tep_get_all_get_params(['cID', 'action']).'cID='.$cInfo->customers_id.'&action=deleteconfirm')];
              $contents[] = ['text' => TEXT_DELETE_INTRO.'<br /><br /><strong>'.$cInfo->customers_firstname.' '.$cInfo->customers_lastname.'</strong>'];

              if (isset($cInfo->number_of_reviews) && $cInfo->number_of_reviews > 0) {
                  $contents[] = ['text' => '<br />'.tep_draw_checkbox_field('delete_reviews', 'on', true).' '.sprintf(TEXT_DELETE_REVIEWS, $cInfo->number_of_reviews)];
              }

              $contents[] = ['align' => 'center', 'text' => '<br />'.tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary').tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers.php', tep_get_all_get_params(['cID', 'action']).'cID='.$cInfo->customers_id))];
          ...
          default:
              if (isset($cInfo) && \is_object($cInfo)) {
                  ...
                  $contents[] = ['align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('customers.php', tep_get_all_get_params(['cID', 'action']).'cID='.$cInfo->customers_id.'&action=edit')).tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('customers.php', tep_get_all_get_params(['cID', 'action']).'cID='.$cInfo->customers_id.'&action=confirm')).tep_draw_button(IMAGE_ORDERS, 'cart', tep_href_link('orders.php', 'cID='.$cInfo->customers_id)).tep_draw_button(IMAGE_EMAIL, 'mail-closed', tep_href_link('mail.php', 'customer='.$cInfo->customers_email_address))];
