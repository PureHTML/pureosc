...
function tep_get_path($current_category_id = '') {
  ...
  if ($current_category_id == '') {
    ...
    if (!isset($cPath_array) || (sizeof($cPath_array) == 0)) {
      ...
      $cPath_new = '';
      $last_category_query = tep_db_query("select parent_id from categories where categories_id = '" . (int)$cPath_array[(sizeof($cPath_array) - 1)] . "'");
...
function tep_resize_image($old_image, $new_image, $width = null, $height = null, $quality = 90) {
  ...
  if ($image_type !== false) {
    list($orig_width, $orig_height) = getimagesize($old_image);
