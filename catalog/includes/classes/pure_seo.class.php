...
class pure_seo
{
    public $parA = [], $langId = 0, $langCode = '', $suspend = false;
    ...
    //..a dal uz pekne po nasem:
        ...
        if ((! in_array($page, $pages)) || $this->suspend) { //na ost. se sere - stejne tak pri vynuc.potlaceni
            $add = '';
...
//--------------------------------------------------------------------------------------
    ...
    //vraci rewr.jmeno vyrobce
        ...
        return ($seoName ? $seoName : ('bez-nakladatele-' . $manufId)) . '/';
    ...
    //vraci rewr.jmeno autora
        ...
        return ($seoName ? $seoName : 'bez-autora') . '/';
    ...
    //komplet prepracovani surovin do fin.podoby
        ...
        $ret = preg_replace(['/^(\-{1})/', '/(\-{1})$/'], '', $ret); //vyhodit ev.pomlcky na zac. a konci
        return $ret;
    }
} // END class
