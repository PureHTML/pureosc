...
class admin_orders_action
{
    public $server = 1;

    public function braintree_hook_admin_orders_action(): void
    {
    ...
    public function getTransactionDetails($comments, $order): void
    {
        global $messageStack;
    ...
    public function doCapture($comments, $order): void
    {
        global $messageStack;
    ...
    public function doVoid($comments, $order): void
    {
        global $messageStack;
    ...
    public function refundTransaction($comments, $order): void
    {
        global $messageStack;
        ...
        if (($error === false) && \is_object($response) && (\get_class($response) === 'Braintree\\Result\\Successful') && ($response->success === true) && (\get_class($response->transaction) === 'Braintree\\Transaction') && isset($response->transaction->refundedTransactionId) && ($response->transaction->refundedTransactionId === $comments['Transaction ID'])) {
            ...
            $messageStack->add_session($this->_app->getDef('ms_success_doRefund', [
                'refund_amount' => tep_db_prepare_input($response->transaction->amount),
            ]), 'success');
        } else {
            $messageStack->add_session($this->_app->getDef('ms_error_doRefund', [
                'refund_amount' => tep_db_prepare_input($response->transaction->amount),
            ]), 'error');
        }
    }
