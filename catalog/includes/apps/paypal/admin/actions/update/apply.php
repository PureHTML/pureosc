...
if (isset($_GET['v']) && is_numeric($_GET['v']) && ($_GET['v'] > $OSCOM_PayPal->getVersion())) {
    ...
    if (file_exists($update_zip)) {
        ...
        if (file_exists($update_pkg) && file_exists($update_pkg.'.sig')) {
            $public = openssl_pkey_get_public(file_get_contents(DIR_FS_CATALOG.'includes/apps/paypal/work/paypal.pubkey'));
            ...
            if (openssl_verify(sha1_file($update_pkg), file_get_contents($update_pkg.'.sig'), $public) === 1) {
                ...
                if ($meta_pass === true) {
                    ...
                    if (empty($errors)) {
                        $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_update_started', ['version' => $update_version]), $update_version);
                        ...
                        foreach ($update_pkg_contents as $file) {
                            ...
                            if (substr($pathname, 0, 8) === 'catalog/') {
                                ...
                                if (copy($file, DIR_FS_CATALOG.$target.basename($pathname))) {
                                    $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_updated_file', ['filepath' => $OSCOM_PayPal->displayPath(DIR_FS_CATALOG.$target.basename($pathname))]), $update_version);
                                } else {
                                ...
                                if (copy($file, DIR_FS_ADMIN.$target.basename($pathname))) {
                                    $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_updated_file', ['filepath' => $OSCOM_PayPal->displayPath(DIR_FS_ADMIN.$target.basename($pathname))]), $update_version);
                                } else {
                        ...
                        if (empty($errors)) {
                            if (file_put_contents(DIR_FS_CATALOG.'includes/apps/paypal/version.txt', $update_version)) {
                                $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_updated_file', ['filepath' => $OSCOM_PayPal->displayPath(DIR_FS_CATALOG.'includes/apps/paypal/version.txt')]), $update_version);
                            } else {
                                $errors[] = $OSCOM_PayPal->displayPath(DIR_FS_CATALOG.'includes/apps/paypal/version.txt');
                            }
                        }
                        ...
                        if (empty($errors)) {
                            $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_update_success', ['version' => $update_version]), $update_version);

                            $ppUpdateApplyResult['rpcStatus'] = 1;
                        } else {
                            $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('log_update_failed', ['version' => $update_version]), $update_version);
                        }
                    }
        ...
        $OSCOM_PayPal->logUpdate($OSCOM_PayPal->getDef('error_log_nonexisting', ['filepath' => $update_zip]), $update_version);
    }
}
