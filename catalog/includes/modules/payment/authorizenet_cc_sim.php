...
class authorizenet_cc_sim
{
    ...
    public function __construct()
    {
        ...
        if ($this->enabled === true) {
            if (empty(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_LOGIN_ID) || empty(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_KEY)) {
    ...
    public function update_status(): void
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ZONE > 0)) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from zones_to_geo_zones where geo_zone_id = '".MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ZONE."' and zone_country_id = '".(int) $order->billing['country']['id']."' order by zone_id");
    ...
    public function process_button()
    {
        ...
        $params = ['x_login' => substr(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_LOGIN_ID, 0, 20),
    ...
    public function before_process(): void
    {
        ...
        if ($error === false) {
            if (($_POST['x_response_code'] === '1') || ($_POST['x_response_code'] === '4')) {
                if (!empty(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_MD5_HASH) && (!isset($_POST['x_MD5_Hash']) || (strtoupper($_POST['x_MD5_Hash']) !== strtoupper(md5(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_MD5_HASH.MODULE_PAYMENT_AUTHORIZENET_CC_SIM_LOGIN_ID.$_POST['x_trans_id'].$this->format_raw($order->info['total'])))))) {
                    $error = 'verification';
...
EOD;
    ...
    public function get_error()
    {
        ...
        $error = ['title' => MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ERROR_TITLE,
            'error' => $error_message];
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = tep_db_query("select configuration_value from configuration where configuration_key = 'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_STATUS'");
    ...
    public function getParams()
    {
        ...
        return ['MODULE_PAYMENT_AUTHORIZENET_CC_SIM_STATUS' => ['title' => 'Enable Authorize.net Server Integration Method',
            'desc' => 'Do you want to accept Authorize.net Server Integration Method payments?',
            'value' => 'True',
            'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_LOGIN_ID' => ['title' => 'API Login ID',
                'desc' => 'The API Login ID used for the Authorize.net service'],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_KEY' => ['title' => 'API Transaction Key',
                'desc' => 'The API Transaction Key used for the Authorize.net service'],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_MD5_HASH' => ['title' => 'MD5 Hash',
                'desc' => 'The MD5 Hash value to verify transactions with'],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_METHOD' => ['title' => 'Transaction Method',
                'desc' => 'The processing method to use for each transaction.',
                'value' => 'Authorization',
                'set_func' => 'tep_cfg_select_option(array(\'Authorization\', \'Capture\'), '],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ORDER_STATUS_ID' => ['title' => 'Set Order Status',
                'desc' => 'Set the status of orders made with this payment module to this value',
                'value' => '0',
                'use_func' => 'tep_get_order_status_name',
                'set_func' => 'tep_cfg_pull_down_order_statuses('],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_REVIEW_ORDER_STATUS_ID' => ['title' => 'Review Order Status',
                'desc' => 'Set the status of orders flagged as being under review to this value',
                'value' => '0',
                'use_func' => 'tep_get_order_status_name',
                'set_func' => 'tep_cfg_pull_down_order_statuses('],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_ORDER_STATUS_ID' => ['title' => 'Transaction Order Status',
                'desc' => 'Include transaction information in this order status level',
                'value' => $status_id,
                'use_func' => 'tep_get_order_status_name',
                'set_func' => 'tep_cfg_pull_down_order_statuses('],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_ZONE' => ['title' => 'Payment Zone',
                'desc' => 'If a zone is selected, only enable this payment method for that zone.',
                'value' => '0',
                'set_func' => 'tep_cfg_pull_down_zone_classes(',
                'use_func' => 'tep_get_zone_class_title'],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_SERVER' => ['title' => 'Transaction Server',
            ...
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_TRANSACTION_MODE' => ['title' => 'Transaction Mode',
                'desc' => 'Transaction mode used for processing orders',
                'value' => 'Live',
                'set_func' => 'tep_cfg_select_option(array(\'Live\', \'Test\'), '],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_DEBUG_EMAIL' => ['title' => 'Debug E-Mail Address',
                'desc' => 'All parameters of an invalid transaction will be sent to this email address.'],
            'MODULE_PAYMENT_AUTHORIZENET_CC_SIM_SORT_ORDER' => ['title' => 'Sort order of display.',
    ...
    {
        if (!empty(MODULE_PAYMENT_AUTHORIZENET_CC_SIM_DEBUG_EMAIL)) {
            $email_body = '';
