...
class paypal_standard
{
    ...
    public function __construct()
    {
        ...
        $this->enabled = \defined('OSCOM_APP_PAYPAL_PS_STATUS') && \in_array(OSCOM_APP_PAYPAL_PS_STATUS, ['1', '0'], true) ? true : false;
        ...
        if ($this->enabled === true) {
            if (!\defined('OSCOM_APP_PAYPAL_PS_PDT_IDENTITY_TOKEN') || (empty(OSCOM_APP_PAYPAL_PS_PDT_IDENTITY_TOKEN) && !$this->_app->hasCredentials('PS'))) {
...
EOD.(int) $languages_id."'";
    ...
    public function process_button()
    {
        ...
        if (\is_array($order_total_modules->modules)) {
            foreach ($order_total_modules->modules as $value) {
                ...
                if ($GLOBALS[$class]->enabled) {
                    for ($i = 0, $n = \count($GLOBALS[$class]->output); $i < $n; ++$i) {
                        if (!empty($GLOBALS[$class]->output[$i]['title']) && !empty($GLOBALS[$class]->output[$i]['text'])) {
                            if (!\in_array($GLOBALS[$class]->code, ['ot_subtotal', 'ot_shipping', 'ot_tax', 'ot_total'], true)) {
        ...
        if (OSCOM_APP_PAYPAL_PS_EWP_STATUS === '1') {
            ...
            $random_string = mt_rand(100000, 999999).'-'.$customer_id.'-';
            ...
            $fp = fopen(OSCOM_APP_PAYPAL_PS_EWP_WORKING_DIRECTORY.'/'.$random_string.'data.txt', 'wb');
            fwrite($fp, $data);
            fclose($fp);
            ...
            if (\function_exists('openssl_pkcs7_sign') && \function_exists('openssl_pkcs7_encrypt')) {
                ...
                $signed = explode("\n\n", $signed);
                $signed = base64_decode($signed[1], true);

                $fp = fopen(OSCOM_APP_PAYPAL_PS_EWP_WORKING_DIRECTORY.'/'.$random_string.'signed.txt', 'wb');
                fwrite($fp, $signed);
                fclose($fp);
    ...
    public function pre_before_check(): void
    {
        ...
        if (!empty($this->_app->getCredentials('PS', 'email_primary'))) {
        ...
        if ((isset($_POST['receiver_email']) && \in_array($_POST['receiver_email'], $seller_accounts, true)) || (isset($_POST['business']) && \in_array($_POST['business'], $seller_accounts, true))) {
            ...
            if (!empty(OSCOM_APP_PAYPAL_PS_PDT_IDENTITY_TOKEN)) {
                ...
                $details = $this->_app->getApiResult('PS', 'GetTransactionDetails', ['TRANSACTIONID' => stripslashes($_GET['tx'])], (OSCOM_APP_PAYPAL_PS_STATUS === '1') ? 'live' : 'sandbox');
...
EOD.(int) $languages_id."'";
    ...
    public function after_process(): void
    {
        ...
        unset($_SESSION['sendto'], $_SESSION['billto'], $_SESSION['shipping'], $_SESSION['payment'], $_SESSION['comments'], $_SESSION['cart_PayPal_Standard_ID']);
    ...
    {
        return tep_db_num_rows(tep_db_query("SELECT configuration_value FROM configuration WHERE configuration_key = 'OSCOM_APP_PAYPAL_PS_STATUS'"));
    }
    ...
    public function verifyTransaction($pptx_params, $is_ipn = false): void
    {
        global $currencies;
