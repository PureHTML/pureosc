...
class stripe
{
    ...
    public function update_status(): void
    {
        ...
        if (($this->enabled === true) && ((int) MODULE_PAYMENT_STRIPE_ZONE > 0)) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from zones_to_geo_zones where geo_zone_id = '".MODULE_PAYMENT_STRIPE_ZONE."' and zone_country_id = '".(int) $order->delivery['country']['id']."' order by zone_id");
    ...
    public function confirmation()
    {
        ...
        for ($i = 1; $i < 13; ++$i) {
            $months_array[] = ['id' => tep_output_string(sprintf('%02d', $i)),
        ...
        for ($i = $today['year']; $i < $today['year'] + 10; ++$i) {
            $years_array[] = ['id' => tep_output_string(strftime('%Y', mktime(0, 0, 0, 1, 1, $i))),
        ...
        $address = ['address_line1' => $order->billing['street_address'],
        ...
        if (MODULE_PAYMENT_STRIPE_TOKENS === 'True') {
            if (isset($_POST['stripe_card']) && is_numeric($_POST['stripe_card']) && ($_POST['stripe_card'] > 0)) {
                ...
                if (tep_db_num_rows($token_query) === 1) {
                    ...
                    tep_redirect(tep_href_link('checkout_payment.php', 'payment_error='.$this->code.'&error=cardstored'));
                }
            }
        }
    ...
    public function after_process(): void
    {
        ...
        $status_comment = ['Transaction ID: '.$stripe_result['id'],
    ...
    public function get_error()
    {
        ...
        $error = ['title' => MODULE_PAYMENT_STRIPE_ERROR_TITLE,
            'error' => $message];
...
EOD;
        ...
        return ['MODULE_PAYMENT_STRIPE_STATUS' => ['title' => 'Enable Stripe Module',
            'desc' => 'Do you want to accept Stripe payments?',
            'value' => 'True',
            'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '],
            'MODULE_PAYMENT_STRIPE_PUBLISHABLE_KEY' => ['title' => 'Publishable API Key',
                'desc' => 'The Stripe account publishable API key to use.',
                'value' => ''],
            'MODULE_PAYMENT_STRIPE_SECRET_KEY' => ['title' => 'Secret API Key',
                'desc' => 'The Stripe account secret API key to use with the publishable key.',
                'value' => ''],
            'MODULE_PAYMENT_STRIPE_TOKENS' => ['title' => 'Create Tokens',
                'desc' => 'Create and store tokens for card payments customers can use on their next purchase?',
                'value' => 'False',
                'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '],
            'MODULE_PAYMENT_STRIPE_VERIFY_WITH_CVC' => ['title' => 'Verify With CVC',
                'desc' => 'Verify the credit card billing address with the Card Verification Code (CVC)?',
                'value' => 'True',
                'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '],
            'MODULE_PAYMENT_STRIPE_TRANSACTION_METHOD' => ['title' => 'Transaction Method',
                'desc' => 'The processing method to use for each transaction.',
                'value' => 'Authorize',
                'set_func' => 'tep_cfg_select_option(array(\'Authorize\', \'Capture\'), '],
            'MODULE_PAYMENT_STRIPE_ORDER_STATUS_ID' => ['title' => 'Set Order Status',
                'desc' => 'Set the status of orders made with this payment module to this value',
                'value' => '0',
                'use_func' => 'tep_get_order_status_name',
                'set_func' => 'tep_cfg_pull_down_order_statuses('],
            'MODULE_PAYMENT_STRIPE_TRANSACTION_ORDER_STATUS_ID' => ['title' => 'Transaction Order Status',
                'desc' => 'Include transaction information in this order status level',
                'value' => $status_id,
                'set_func' => 'tep_cfg_pull_down_order_statuses(',
                'use_func' => 'tep_get_order_status_name'],
            'MODULE_PAYMENT_STRIPE_ZONE' => ['title' => 'Payment Zone',
                'desc' => 'If a zone is selected, only enable this payment method for that zone.',
                'value' => '0',
                'use_func' => 'tep_get_zone_class_title',
                'set_func' => 'tep_cfg_pull_down_zone_classes('],
            'MODULE_PAYMENT_STRIPE_TRANSACTION_SERVER' => ['title' => 'Transaction Server',
    ...
    public function sendTransactionToGateway($url, $parameters = null, $curl_opts = [])
    {
        ...
        $header = ['Stripe-Version: '.$this->api_version,
...
EOD;
        ...
        return '<p><img src="images/icons/locked.gif" border="0">&nbsp;<a href="javascript:openTestConnectionDialog();" style="text-decoration: underline; font-weight: bold;">'.MODULE_PAYMENT_STRIPE_DIALOG_CONNECTION_LINK_TITLE.'</a></p>'.
