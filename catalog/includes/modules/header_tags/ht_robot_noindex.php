...
class ht_robot_noindex
{
    ...
    public function execute(): void
    {
        ...
        if (!empty(MODULE_HEADER_TAGS_ROBOT_NOINDEX_PAGES)) {
            ...
            if (\in_array(basename($PHP_SELF), $pages_array, true)) {
    ...
    public function install(): void
    {
        ...
        tep_db_query("insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_HEADER_TAGS_ROBOT_NOINDEX_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
...
function ht_robot_noindex_edit_pages($values, $key)
{
    ...
    foreach ($files_array as $file) {
        $output .= tep_draw_checkbox_field('ht_robot_noindex_file[]', $file, \in_array($file, $values_array, true)).'&nbsp;'.tep_output_string($file).'<br />';
    }
