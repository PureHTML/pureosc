...
class standard extends uri_modules
{
    ...
    public function isValidUri()
    {
        ...
        if (false !== strpos(usu5::i()->getVar('request_uri'), '.html')) { // Standard seo url does not have .html
            return false;
        }

        // Not a path based uri
        if (false !== strpos(usu5::i()->getVar('request_uri'), '/')) { // Standard seo url does not have a / in the uri
