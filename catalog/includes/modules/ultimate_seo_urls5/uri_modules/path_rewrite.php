...
class path_rewrite extends uri_modules
{
    ...
    public function isValidUri()
    {
        ...
        if (false === strpos(usu5::i()->getVar('request_uri'), '.html')) { // path_rewrite seo url must have .html
            return false;
        }

        // Is a path based uri
        if (false === strpos(usu5::i()->getVar('request_uri'), '/')) { // path_rewrite seo url must have a / in the uri
            return false;
        }
