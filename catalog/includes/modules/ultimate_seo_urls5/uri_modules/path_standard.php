...
class path_standard extends uri_modules
{
    ...
    public function isValidUri()
    {
        ...
        // Not a rewrite uri
        if (false !== strpos(Usu_Main::i()->getVar('request_uri'), '.html')) { // path_standard seo url does not have .html
            return false;
        }

        // Is a path based uri
        if (false === strpos(Usu_Main::i()->getVar('request_uri'), '/')) { // path_standard seo url must have a / in the uri
            return false;
        }
