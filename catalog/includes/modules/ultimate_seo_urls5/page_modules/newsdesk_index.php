...
class newsdesk_index extends page_modules
{
     ...
     */
    protected array $dependencies = ['newspath' => ['marker' => '-nc-',
        ...
        'to_replace' => [':TABLE_NEWSDESK_CATEGORIES_DESCRIPTION', ':newspath', ':languages_id']]];
     ...
     */
    protected array $cache_name_builder = ['newspath' => 1]; // xxx = _GET key ( e.g. cPath ), you may want to add "page" if there are paging functions
     ...
     *
     * @return Newsdesk_Index_Page_Module
     */
    ...
    public function buildLink($page, $parameters, $add_session_id, $connection)
    {
        ...
        switch (true) {
            case $this->key === 'newspath': // xxx = _GET key ( e.g. cPath )
                // This array contains replacements for the to_replace array ( see the $dependencies array )
                $this->setQuery([TABLE_NEWSDESK_CATEGORIES_DESCRIPTION, $this->stripPathToLastNumber($this->keys_index[$this->key]), Usu_Main::i()->getVar('languages_id')]);
