...
/**
 * Page module for the articles contribution.
 */
class articles extends aPage_Modules
{
    /**
     ...
     */
    protected array $cache_name_builder = ['tPath' => 1, 'authors_id' => 1]; // xxx = _GET key ( e.g. cPath ), you may want to add "page" if there are paging functions
    ...
    public function buildLink($page, $parameters, $add_session_id, $connection)
    {
        ...
        switch (true) {
            case $this->key === 'tPath': // xxx = _GET key ( e.g. cPath )
                // This array contains replacements for the to_replace array ( see the $dependencies array )
                $this->setQuery([TABLE_TOPICS_DESCRIPTION, $this->stripPathToLastNumber($this->keys_index[$this->key]), Usu_Main::i()->getVar('languages_id')]);

                break;
            case $this->key === 'authors_id': // xxx = _GET key ( e.g. cPath )
                // This array contains replacements for the to_replace array ( see the $dependencies array )
                $this->setQuery([TABLE_AUTHORS, $this->stripPathToLastNumber($this->keys_index[$this->key])]);
