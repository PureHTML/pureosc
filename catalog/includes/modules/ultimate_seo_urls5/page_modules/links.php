...
/**
 * Page module for links manager.
 */
class links extends aPage_Modules
{
    /**
     ...
     */
    protected array $dependencies = ['lPath' => ['marker' => '-links-',
        ...
        'to_replace' => [':TABLE_LINK_CATEGORIES_DESCRIPTION', ':lPath', ':languages_id']]];
     ...
     */
    protected array $cache_name_builder = ['lPath' => 1]; // xxx = _GET key ( e.g. cPath ), you may want to add "page" if there are paging functions
    ...
    public function buildLink($page, $parameters, $add_session_id, $connection)
    {
        ...
        switch (true) {
            case $this->key === 'lPath': // xxx = _GET key ( e.g. cPath )
                // This array contains replacements for the to_replace array ( see the $dependencies array )
                $this->setQuery([TABLE_LINK_CATEGORIES_DESCRIPTION, $this->stripPathToLastNumber($this->keys_index[$this->key]), Usu_Main::i()->getVar('languages_id')]);
