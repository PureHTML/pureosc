...
/**
 * Page module for newsdesk reviews article.
 */
class newsdesk_reviews_article extends aPage_Modules
{
    /**
     ...
     */
    protected array $dependencies = ['newsdesk_id' => ['marker' => '-nra-',
        ...
        'to_replace' => [':TABLE_NEWSDESK_DESCRIPTION', ':newsdesk_id', ':languages_id']]];
     ...
     */
    protected array $cache_name_builder = ['newsdesk_id' => 1]; // xxx = _GET key ( e.g. cPath ), you may want to add "page" if there are paging functions
     ...
     *
     * @return Newsdesk_Reviews_Article_Page_Module
     */
    ...
    public function buildLink($page, $parameters, $add_session_id, $connection)
    {
        ...
        switch (true) {
            case $this->key === 'newsdesk_id': // xxx = _GET key ( e.g. cPath )
                // This array contains replacements for the to_replace array ( see the $dependencies array )
                $this->setQuery([TABLE_NEWSDESK_DESCRIPTION, $this->stripPathToLastNumber($this->keys_index[$this->key]), Usu_Main::i()->getVar('languages_id')]);
