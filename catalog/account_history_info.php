...
EOD.$order->products[$i]['qty'].<<<'EOD'
&nbsp;x&nbsp;</td>
                <td>
EOD.$order->products[$i]['name'];
          ...
          echo '  <td class="text-end">'.$currencies->format(tep_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']) * $order->products[$i]['qty'], true, $order->info['currency'], $order->info['currency_value']).<<<'EOD'
</td>
            </tr>
EOD;
      }
...
EOD.$order->totals[$i]['title'].<<<'EOD'
</span></td>
                <td class="text-end">
EOD.$order->totals[$i]['text'].<<<'EOD'
</td>
              </tr>
EOD;
}
...
<tr>
                <td>
EOD.tep_date_short($statuses['date_added']).<<<'EOD'
</td>
                <td>
EOD.$statuses['orders_status_name'].<<<'EOD'
</td>
                <td class="w-75">
EOD.(empty($statuses['comments']) ? '&nbsp;' : nl2br(tep_output_string_protected($statuses['comments']))).<<<'EOD'
</td>
              </tr>
EOD;
}
...
?>
    ...
    <?php echo tep_draw_button(IMAGE_BUTTON_BACK, 'triangle-1-w', tep_href_link('account_history.php', tep_get_all_get_params(['order_id'])), 'btn-light'); ?>
