...
if (!isset($_GET['installation']) || ($_GET['installation'] !== MODULE_PAYMENT_RBSWORLDPAY_HOSTED_INSTALLATION_ID)) {
    ...
    $error = true;
} elseif (!empty(MODULE_PAYMENT_RBSWORLDPAY_HOSTED_CALLBACK_PASSWORD) && (!isset($_POST['callbackPW']) || ($_POST['callbackPW'] !== MODULE_PAYMENT_RBSWORLDPAY_HOSTED_CALLBACK_PASSWORD))) {
    $error = true;
...
if ($order['orders_status'] === MODULE_PAYMENT_RBSWORLDPAY_HOSTED_PREPARE_ORDER_STATUS_ID) {
    ...
    $sql_data_array = ['orders_id' => $order['orders_id'],
...
<form action="<?php echo tep_href_link('checkout_process.php', tep_session_name().'='.$_POST['M_sid'].'&hash='.$_POST['M_hash'], 'SSL', false); ?>" method="post" target="_top">
