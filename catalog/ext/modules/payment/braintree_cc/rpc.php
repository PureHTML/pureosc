...
if (!\defined('OSCOM_APP_PAYPAL_BRAINTREE_CC_STATUS') || !\in_array(OSCOM_APP_PAYPAL_BRAINTREE_CC_STATUS, ['1', '0'], true)) {
    exit;
}
...
switch ($_GET['action']) {
    case 'paypal':
        if (
            ...
            && isset($_SESSION['appBraintreeCcFormHash'], $_POST['bt_paypal_form_hash'])
    ...
    case 'getCardToken':
        ...
        if (tep_db_num_rows($card_query)) {
            ...
            $pmn = Braintree\PaymentMethodNonce::create($card['braintree_token']);

            $result = ['result' => 1, 'token' => $pmn->paymentMethodNonce->nonce];
        }
