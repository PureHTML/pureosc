...
if (DISPLAY_PRICE_WITH_TAX === 'true') {
    if ($pfrom > 0) {
        $where_str .= ' and (IF(s.status, s.specials_new_products_price, p.products_price) * if(gz.geo_zone_id is null, 1, 1 + (tr.tax_rate / 100) ) >= '.(float) $pfrom.')';
    }

    if ($pto > 0) {
        $where_str .= ' and (IF(s.status, s.specials_new_products_price, p.products_price) * if(gz.geo_zone_id is null, 1, 1 + (tr.tax_rate / 100) ) <= '.(float) $pto.')';
    }
} else {
    if ($pfrom > 0) {
        $where_str .= ' and (IF(s.status, s.specials_new_products_price, p.products_price) >= '.(float) $pfrom.')';
    }

    if ($pto > 0) {
        $where_str .= ' and (IF(s.status, s.specials_new_products_price, p.products_price) <= '.(float) $pto.')';
    }
}
